...
public class TimeUtil {
	 ...
	 * @return
	 */
	public static String dateRemainDay(Date targetDate){
		Calendar calendar = Calendar.getInstance();
		calendar.set(calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH), calendar.get(Calendar.DATE), 0, 0, 0);
		Date today = calendar.getTime();
		...
		int dayNumber = (int) ((target.getTime()-today.getTime())/86400000);
		if(dayNumber <0){
			return "时间已过期";
		}
		else{
			return "到截止日期还剩"+dayNumber+"天";
		}
	}
	
	public static String dateRemainDay(String dateStr){
		SimpleDateFormat sdf = new SimpleDateFormat(PATTERN_TIME);
		try {
			Date date = sdf.parse(dateStr);
			return dateRemainDay(date);
