...
import com.wutong.common.widget.HorizontalSmoothScrollView;
...
public class RepairOrderDetailActivity extends BaseFragmentActivity {
	...
	private final int LARGE_CHARATER_REQUEST_CODE = 99; 
	private final int HOW_REPAIR_REQUEST_CODE = 77;
	
	private ImageView titlebarBackView;
	private TextView titlebarTitleView;
	private ImageView titlebarMoreView;
	...
	private String lastFragmentOrActivity;
	...
	private CustomDialog operationView;
	...
	private CustomDialog operationCustomDialog;
	private Vector<String> operationNameArray;
	private Vector<Integer> operationIndex;
	private Map<String, Integer> operationNameIndexMap;
	
	private OperationWheelAdapter operationWheelAdapter;
	private View operationContentView;
	private WheelView operationWv;
	private String[] operationItems;
	

	private final static String OPERATION_ASSIGNMENT = "派工";
	private final static int OPERATION_ASSIGNMENT_INDEX = 1;
	private final static String OPERATION_REMIND = "催单";
	private final static int OPERATION_REMIND_INDEX = 2;

	private final static String OPERATION_CHECK_AND_QUERY = "确认验收";
	private final static int OPERATION_CHECK_AND_QUERY_INDEX = 3;
	private final static String OPERATION_APPLY_REFIX = "申请返修";
	private final static int OPERATION_APPLY_REFIX_INDEX = 4;

	private final static String OPERATION_ACCEPT_ORDER = "受理报修单";
	private final static int OPERATION_ACCEPT_ORDER_INDEX = 5;
	private final static String OPERATION_FORWARD_ORDER = "大额度报修单转发";
	private final static int OPERATION_FORWARD_ORDER_INDEX = 6;
	private final static String OPERATION_FINISH_ORDER = "完成报修单";
	private final static int OPERATION_FINISH_ORDER_INDEX = 7;
	private final static String OPERATION_REFIX_FINISH_ORDER = "完成报修单（返修）";
	private final static int OPERATION_REFIX_FINISH_ORDER_INDEX = 8;
	...
	private CustomDialog queryCheckAndSubmitDialog;
	private CustomDialog queryNeedRefixDialog;
	private RadioGroup assessView;
...
//	private RadioGroup mFastGroupView;
//	private RadioButton mFastQueryAcceptRadioView;
//	private RadioButton mFastRebackRadioView;
//	private TextView mFastAssessSelect;
//	private EditText mFastResonView;
//	private List<AssessBean> assessBeanList;
//	private String mFastAssessCode;
//	private SingleSelectDialogFragment mFastAssessSelectDialog;
	private void mapDataInit(){
		operationNameArray = new Vector<String>();
		operationIndex = new Vector<Integer>();
		operationNameIndexMap = new HashMap<String, Integer>();
		operationNameIndexMap.put(OPERATION_ASSIGNMENT, OPERATION_ASSIGNMENT_INDEX);
		operationNameIndexMap.put(OPERATION_REMIND, OPERATION_REMIND_INDEX);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		outIsLargeCharacter = getIntent().getBooleanExtra("isLargeCharacter",false);
		if(outIsLargeCharacter){
			Intent intentForword = new Intent(mContext,RepairOrderDetailLargeActivity.class);
			intentForword.putExtras(getIntent());
			startActivityForResult(intentForword, LARGE_CHARATER_REQUEST_CODE);
			return;
		}
		commonInit();
		intentInit();
...
//		assessBeanList = new ArrayList<AssessBean>();
	...
	private void titlebarInit(){
		...
		titlebarSubmitView.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
//				queryAcceptOrRebackFast();
			}
		});
		titlebarMoreView = (ImageView) findViewById(R.id.titlebar_more);
		titlebarMoreView.setOnClickListener(new OnClickListener() {

			@Override
			public void onClick(View v) {
				if(operationCustomDialog != null && !operationCustomDialog.isShowing()){
					operationCustomDialog.show();
				}
			}
		});
	...
	private void intentInit(){
		...
		lastFragmentOrActivity = intent.getStringExtra("lastFragmentOrActivity");
		Logger.i("lastFragmentOrActivity:" +lastFragmentOrActivity);
	}

	private void viewInit(){
		assignmentAlertContentView = LayoutInflater.from(mContext).inflate(R.layout.common_single_list_select_no_button, null);
		...
		assessView = (RadioGroup) LayoutInflater.from(mContext).inflate(R.layout.alertdialog_assess,null);
		...
		//
//		mFastLayout = findViewById(R.id.repair_order_detail_fast_layout);
//		mFastGroupView = (RadioGroup) findViewById(R.id.repair_order_detail_fast_group);
//		mFastQueryAcceptRadioView = (RadioButton) findViewById(R.id.repair_order_detail_fast_query_accept);
//		mFastRebackRadioView = (RadioButton) findViewById(R.id.repair_order_detail_fast_reback);
//		mFastAssessSelect = (TextView) findViewById(R.id.repair_order_detail_fast_assess_select);
//		mFastResonView = (EditText) findViewById(R.id.repair_order_detail_fast_reback_reason_input);
//
//		mFastAssessSelect.setOnClickListener(new OnClickListener() {
//			
//			@Override
//			public void onClick(View v) {
//				mFastAssessSelectDialog = new SingleSelectDialogFragment();
//				Bundle data = new Bundle();
//				data.putSerializable("dataList", (Serializable) assessBeanList);
//				mFastAssessSelectDialog.setArguments(data);
//				mFastAssessSelectDialog.setDataChangeListener(new DataChangeListener() {
//
//					@Override
//					public void onDataChange(SingleSelectable itemBean) {
//						mFastAssessSelect.setText(itemBean.getTextName());
//						mFastAssessCode = itemBean.getValue();
//					}
//				});
//				mFastAssessSelectDialog.show(getSupportFragmentManager(), "single-select");
//			}
//		});
	...
	protected void onActivityResult(int requestCode, int resultCode, final Intent data) {
		...
		if (resultCode == Activity.RESULT_OK ) {
			if(requestCode == TAKE_PHOTOS_REQUEST_CODE){
			...
			else if(requestCode == LARGE_CHARATER_REQUEST_CODE){
				...
				File file = CommonSwitcherUtil.getAbsoluteImagePath(RepairOrderDetailActivity.this, uri);
				...
				String assessCode = data.getStringExtra("data");
				queryAndAccept(assessCode);
			}
			...
			if(requestCode == LARGE_CHARATER_REQUEST_CODE){
				finish();
			}
			else{
	 ...
	 * 确认验收改正
	 */
	private void queryAndAccept(final String assessCode){
		...
		changeStatusAsyncHttp(repairOrderStatusFormBean,new ChangeStatusListener() {
			...
			public void onSuccess() {
				...
				assessBean.setAssessTypeCode(assessCode);
	...
	private void loadRepairOrderDetailAsyncHttp(String repairOrderId){
		...
		httpAsyncTask.setOnDealtListener(new OnDealtListener() {
			...
			public void success(String resultResponse) {
				...
				//界面显示和隐藏
				findViewById(R.id.repair_detail_contact_layout).setVisibility(ViewGroup.VISIBLE);
				findViewById(R.id.repair_detail_contact_line).setVisibility(ViewGroup.VISIBLE);
				...
				operationNameArray.clear();
				operationIndex.clear();
				if(lastFragmentOrActivity != null && lastFragmentOrActivity.equals(EmploymentRepairGroupListActivity.class.getName())
						&& application.hasPermission(getString(R.string.roles_permissions_manager_assignment_order))
						&& orderStatus != 4 && orderStatus != 7 && orderStatus != 3 && orderStatus != 6){
					//派工
					operationNameArray.add(OPERATION_ASSIGNMENT);
					operationIndex.add(OPERATION_ASSIGNMENT_INDEX);
				}
				if(lastFragmentOrActivity != null && lastFragmentOrActivity.equals(EmploymentRepairGroupListActivity.class.getName())
						&& application.hasPermission(getString(R.string.roles_permissions_manager_remind_repairman))
						&&(repairmanId !=null && repairmanId.toString().trim().length() > 0 && !repairmanId.equals("0"))
						&& orderStatus != 4 && orderStatus != 7 && orderStatus != 3 && orderStatus != 6) {
					//催单(附加 有维修工id + 状态不是已确认和返修确认、返修完成、已维修)
					operationNameArray.add(OPERATION_REMIND);
					operationIndex.add(OPERATION_REMIND_INDEX);
				}
				//Fix分支
					if(application.hasPermission(getString(R.string.roles_permissions_applicant_query_and_check))
						...
						//确认验收
						operationNameArray.add(OPERATION_CHECK_AND_QUERY);
						operationIndex.add(OPERATION_CHECK_AND_QUERY_INDEX);
					}
				if(application.hasPermission(getString(R.string.roles_permissions_applicant_apply_refix))
						...
						&&resultBean.getFormType().equals(RepairTypeDict.DORM_TYPE)){
					//申请返修
					operationNameArray.add(OPERATION_APPLY_REFIX);
					operationIndex.add(OPERATION_APPLY_REFIX_INDEX);
				}

				if(application.hasPermission(getString(R.string.roles_permissions_repairman_accept_order))
						&&orderStatus == 1){
					//受理报修单
					operationNameArray.add(OPERATION_ACCEPT_ORDER);
					operationIndex.add(OPERATION_ACCEPT_ORDER_INDEX);
				}

				if(application.hasPermission(getString(R.string.roles_permissions_repairman_forward_order))
						&&orderStatus == 1){
					//大额度报修单
					operationNameArray.add(OPERATION_FORWARD_ORDER);
					operationIndex.add(OPERATION_FORWARD_ORDER_INDEX);
				}

				if(application.hasPermission(getString(R.string.roles_permissions_repairman_finish_order))
						&&orderStatus == 2){
					//完成报修单
					operationNameArray.add(OPERATION_FINISH_ORDER);
					operationIndex.add(OPERATION_FINISH_ORDER_INDEX);
				}

				if(application.hasPermission(getString(R.string.roles_permissions_repairman_refix_finish_order))
						&&orderStatus == -1){
					//完成报修单（返修）
					operationNameArray.add(OPERATION_REFIX_FINISH_ORDER);
					operationIndex.add(OPERATION_REFIX_FINISH_ORDER_INDEX);
				}


				operationItems = operationNameArray.toArray(new String[]{});
				if(operationItems.length >0){
					titlebarMoreView.setVisibility(ViewGroup.VISIBLE);
				}
				else{
					titlebarMoreView.setVisibility(ViewGroup.GONE);
				}
				//快速验收和返修
				if(application.hasPermission(getString(R.string.roles_permissions_fast_applicant_query_and_checkand_refix))
						&&(orderStatus == 3 || orderStatus == 6 )
						&&application.hasRunTimePermission(RUNTIME_PERMISSION_KEY,RuntimePermission.APPLIER_ROLE)
						&&(resultBean.getFormType().equals(RepairTypeDict.DORM_TYPE)||resultBean.getFormType().equals(RepairTypeDict.PUBLIC_INNER_TYPE))){
					titlebarSubmitView.setVisibility(ViewGroup.VISIBLE);
//					mFastLayout.setVisibility(ViewGroup.VISIBLE);
					//隐藏备注和图片区域
					View remarkImgLayout = findViewById(R.id.repair_detail_remark_img_layout);
					if(remarkImgLayout!= null){
						remarkImgLayout.setVisibility(ViewGroup.GONE);
					}
				}
				//
				
				operationContentView = LayoutInflater.from(mContext).inflate(R.layout.common_single_list_select_no_button, null);
				operationWv = (WheelView) operationContentView.findViewById(R.id.select_item_wheel);
				operationWv.setBackgroundResource(R.drawable.common_wheel_bg);
				operationWv.setCenterDrawable(getResources().getDrawable(R.drawable.common_wheel_val));
				operationWheelAdapter = new OperationWheelAdapter();
				operationWv.setViewAdapter(operationWheelAdapter);
				operationCustomDialog = new Builder(mContext)
				.setTitle("操作")
				...
				.setPositiveButton("确认", new DialogInterface.OnClickListener() {
					...
					@Override
					public void onClick(DialogInterface dialog, int which) {
						int wvIndex = operationWv.getCurrentItem();
						int index = operationIndex.get(wvIndex);
						switch (index) {
						case OPERATION_ASSIGNMENT_INDEX:
							assignmentCustomDialog.show();
							break;
						case OPERATION_REMIND_INDEX:
							remindEmployment();
							break;
						case OPERATION_CHECK_AND_QUERY_INDEX:
							checkAndQuery();
							break;
						case OPERATION_APPLY_REFIX_INDEX:
							needRefix();
							break;
						case OPERATION_ACCEPT_ORDER_INDEX:
							checkAndAccept();
							break;
						case OPERATION_FORWARD_ORDER_INDEX:
							muchMoneyForward();
							break;
						case OPERATION_FINISH_ORDER_INDEX:
							complete();
							break;
						case OPERATION_REFIX_FINISH_ORDER_INDEX:
							refixComplete();
							break;
						default:
							break;
		...
		httpAsyncTask.setOnDealtListener(new OnDealtListener() {
			...
			public void success(String resultResponse) {
				...
				if(responseBean.isSuccess()){
					...
					submitSuccess(RepairFragment.CALL_CODE_NOT_ACCEPT_REFRESH);
				}
				else{
...
//		notification.setLatestEventInfo(context,title, content,contentIntent);
	...
	private void subViewSetpuData(RepairOrderBean repairOrderBean){
		...
		mFeedbackCountView.setText(repairOrderBean.getFeedbackCount().toString());
		mRoomAssetInfoView.setText(repairOrderBean.getAssetCount().toString());
		mMaterialInfoView.setText(repairOrderBean.getMaterialCount().toString());
	}
	...
	private class OperationWheelAdapter implements WheelViewAdapter{
		...
		public View getItem(int index, View convertView, ViewGroup parent) {
			convertView = LayoutInflater.from(mContext).inflate(R.layout.wheel_list_item_common, null);
			TextView textView = (TextView) convertView.findViewById(R.id.wheel_item_text); 
			String name = operationItems[index];
	 ...
	 * 快速验收，快速反馈
	 */
//	private void queryAcceptOrRebackFast(){
//		if(mFastGroupView.getCheckedRadioButtonId() == R.id.repair_order_detail_fast_query_accept){
//			if(mFastAssessCode == null){
//				ToastUtil.showToast(mContext, "请评价后提交");
//				return;
//			}
//			queryAndAccept(mFastAssessCode);
//		}
//		else if(mFastGroupView.getCheckedRadioButtonId() == R.id.repair_order_detail_fast_reback){
//			String reason = mFastResonView.getText().toString();
//			needRefixAsyncHttp(repairOrderId,reason);
//		}
//		else{
//			ToastUtil.showToast(mContext, "请选择验收或者返修后再提交");
//		}
//	}
...
//						assessBeanList.addAll(responseList);
	...
	private int selectedContentView(){
		switch (application.getSkinType()) {
		case 1:
			return R.layout.spring_horse_activity_repair_order_detail;
