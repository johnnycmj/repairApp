...
public class HelpFoundSubmitActivity extends BaseFragmentActivity {
	...
	private CheckBox mIsPublicContact;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_help_found_submit);
		commonInit();
		intentInit();
		titlebarInit();
		viewInit();
	...
	private void titlebarInit(){
		...
		titlebarTitleView.setText(R.string.title_activity_found_submit);
		...
		titlebarbackView.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				HelpFoundSubmitActivity.this.onBackPressed();
	...
	private void viewInit(){
		/////
		mRemarkView = (EditText) findViewById(R.id.found_submit_remark);
		
		mAddressView = (EditText) findViewById(R.id.found_submit_address);
		mContactView = (EditText) findViewById(R.id.found_submit_contact);
		mIsPublicContact = (CheckBox) findViewById(R.id.found_submit_is_public_contact);
		...
		mItemTypeSelectView = (TextView) findViewById(R.id.found_submit_type);
		mPlaceSelectView = (TextView) findViewById(R.id.found_submit_street);
		mFoundTimeSelectView = (TextView) findViewById(R.id.found_submit_time);
	...
	private void submit(){
		if(mFoundTimeSelectView.getText().toString().trim().length() == 0){
			ToastUtil.showToast(mContext, "请选择捡到时间");
			return;
		}
		if(mPlaceSelectView.getText().toString().trim().length() == 0){
			ToastUtil.showToast(mContext, "请选择捡到地点");
			return;
		}
	...
	private void submitAsyncHttp(){
		...
		params.put("type", "2");//失物或招领
		params.put("itemType", dataItemType);//物品类别
		params.put("findDate", dataFoundTime);
		params.put("isOpenCollect", mIsPublicContact.isChecked()?"1":"0");
		params.put("phone", application.getLoginInfoBean().getPhone());
		params.put("detail", mContactView.getText().toString());
		params.put("title", "我捡到了");
		...
		application.getFinalHttp().post(url, params, new AjaxCallBackPlus<Object>(){
			...
			public void onSuccess(Object t) {
				super.onSuccess(t);
				hideProgressDialog();
				CommonOperateUtil.hideIME(HelpFoundSubmitActivity.this);
	...
	private void submitSuccess(){
		ToastUtil.showToast(mContext, "已成功发布");
		...
		intent.putExtra(getString(R.string.call_fragment_key_code), HelpFoundListFragment.HELP_FOUND_SUBMIT_REQUEST_CODE);//新投稿刷新未受理
		setResult(RESULT_OK,intent);
		HelpFoundSubmitActivity.this.finish();
	}
	...
	private void uploadPhotoAsyncHttpFile(String repairOrderId,Map<String, String> fileMap){
		...
		httpFileAsyncTask.setOnDealtListener(new HttpFileAsyncTask.OnDealtListener() {
			...
			public void beforeDealt() {
				showNotifcation(mContext,PHOTO_NOTIFICATION_BEFORE_ID, "信息已提交,正在上传图片", "请耐心等候", false,HelpFoundSubmitActivity.class);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode == Activity.RESULT_OK ) {
			if(requestCode == TAKE_PHOTOS_REQUEST_CODE){
				...
				File file = CommonSwitcherUtil.getAbsoluteImagePath(HelpFoundSubmitActivity.this, uri);
