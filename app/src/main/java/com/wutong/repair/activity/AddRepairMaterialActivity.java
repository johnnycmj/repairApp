...
public class AddRepairMaterialActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setStatPageName(mContext, R.string.title_activity_add_repair_material);
		intentInit();
		titlebarInit();
		viewInit();
	}
	...
	private void titlebarInit(){
		...
		titlebarTitleView.setText(R.string.title_activity_add_repair_material);
		...
		titlebarBackView.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				AddRepairMaterialActivity.this.onBackPressed();
	...
	private void viewInit(){
		...
		mSelectMaterailView.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				bundle.putInt("type", SelectRepairMaterialActivity.ADD_TYPE);
	...
	private final class UsedMaterailBaseAdapter extends BaseAdapter{
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if(obj != null){
				...
				final int availableNumber = materialBean.getTotalNumber();//当前可用总数应当是现有剩余

				holder.usedNumberView.setText(getString(R.string.format_repair_material_number,availableNumber));
				...
				holder.minusView.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						//如果大于0，减1
						if(materialBean.getCurrentPlusedNumber().intValue() > 0){
							materialBean.setCurrentPlusedNumber(materialBean.getCurrentPlusedNumber()-1);
							materialBean.setSelectedNumber(lastUsedNumber);
							holder.incrementUsedNumberView.setText(materialBean.getCurrentPlusedNumber().toString());
							materialBean.setImcrementNumber(materialBean.getCurrentPlusedNumber().intValue());
						}
					}
				});
				holder.plusView.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						//如果小于最大可用数，加1
						if(materialBean.getCurrentPlusedNumber().intValue() <availableNumber){
							materialBean.setCurrentPlusedNumber(materialBean.getCurrentPlusedNumber()+1);
							materialBean.setSelectedNumber(lastUsedNumber);
							holder.incrementUsedNumberView.setText(materialBean.getCurrentPlusedNumber().toString());
							materialBean.setImcrementNumber(materialBean.getCurrentPlusedNumber().intValue());
	...
	private void submitSuccess(){
		...
		setResult(RESULT_OK);
		AddRepairMaterialActivity.this.finish();
	}
	...
	private int selectedContentView(){
		switch (application.getSkinType()) {
		case 1:
			return R.layout.spring_horse_activity_add_repair_material;

		default:
			return R.layout.activity_add_repair_material;
		}
	}
}
