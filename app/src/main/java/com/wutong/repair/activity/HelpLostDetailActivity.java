 ...
 *
 */
public class HelpLostDetailActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_help_lost_detail);
		setStatPageName(mContext, R.string.title_help_lost_detail);
		intentInit();
		titleBarInit();
		viewInit();
	}
	...
	private void titleBarInit(){
		...
		titlebarBackView.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				HelpLostDetailActivity.this.onBackPressed();
	...
	private void intentInit(){
		...
		if(isMyPublish){
			mTitle = getString(R.string.title_help_lost_detail_my_publish);
		}
		else{
			mTitle = getString(R.string.title_help_lost_detail);
		}
	}
							...
							public void onClick(View v) {
								...
								if(phone.toString().trim().length() == 0){
									...
									queryDialog = new CustomDialog.Builder(mContext).setTitle("友情提示")
											.setMessage("物主没有公开联系方式，是否联系小微帮忙解决？")
