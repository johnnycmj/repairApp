 ...
 *
 */
public class OfficeMeterFragment extends BaseFragment{
	...
	private TextView selectDivisionUnitView;
	private View selectDivisionUnitContentView;
	private WheelView selectDivisionUnitWv;
	private CustomDialog selectDivisionUnitCustomDialog;
	private RoomMeterWheelAdapter roomMeterWheelAdapter;

	private ListView roomMeterListView;
	private List<RoomMeterBean> roomMeterBeanList;
	private List<RoomMeterBean> roomMeterSelectBeanList;
	private RoomMeterBean roomMeterSelectBean;
	...
	private List<MeterBuildingBean> meterBuildingBeanList;//住户楼

	private List<ChildMeterBean> childMeterBeanList;//单位

	@Override
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
			Bundle savedInstanceState) {
		View fragmentView = inflater.inflate(R.layout.fragment_office_meter, container,false);
	...
	private void dataInit(){
		...
		roomMeterSelectBeanList = new ArrayList<RoomMeterBean>();
		childMeterBeanList = new ArrayList<ChildMeterBean>();
	}
	...
	private void titleBarInit(){
		...
		titlebarBackView.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				OfficeMeterFragment.this.getActivity().onBackPressed();

			}
		});

		titlebarTitleView = (TextView) findViewById(R.id.titlebar_title);
		titlebarTitleView.setText(R.string.title_fragment_office_meter);
	...
	private void viewInit(){
		...
		.setPositiveButton("确认", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				selectDivisionUnitView.setText("");
		...
		.create();
		//
		selectDivisionUnitView = (TextView) findViewById(R.id.select_division_unit);
		selectDivisionUnitContentView = LayoutInflater.from(mContext).inflate(R.layout.common_single_list_select_no_button, null);

		selectDivisionUnitWv = (WheelView) selectDivisionUnitContentView.findViewById(R.id.select_item_wheel);
		selectDivisionUnitWv.setBackgroundResource(R.drawable.common_wheel_bg);
		selectDivisionUnitWv.setCenterDrawable(getResources().getDrawable(R.drawable.common_wheel_val));
		selectDivisionUnitCustomDialog = new CustomDialog.Builder(mContext)
		.setTitle("单位")
		.setView(selectDivisionUnitContentView)
		.setPositiveButton("确认", new DialogInterface.OnClickListener() {

			@Override
			public void onClick(DialogInterface dialog, int which) {
				int index = selectDivisionUnitWv.getCurrentItem();
				roomMeterSelectBean = roomMeterBeanList.get(index);
				roomMeterSelectBeanList.clear();
				roomMeterSelectBeanList.add(roomMeterSelectBean);
				String houseId = roomMeterSelectBean.getOfficeId();
				selectDivisionUnitView.setText(roomMeterSelectBean.getRoomOwnerName() +roomMeterSelectBean.getRoomOwnerCode());
				loadDivisionMeterListHttp(houseId);
				dialog.dismiss();
			}
		})
		.setNegativeButton("取消", new DialogInterface.OnClickListener() {
		...
		selectDivisionUnitView.setOnClickListener(new OnClickListener() {

			@Override
			public void onClick(View v) {
				if(meterBuildingBeanList.size() == 0){
					ToastUtil.showToast(mContext, "没有住宅楼信息");
				}
				else if(selectBuildingView.getText().length() ==0){
					ToastUtil.showToast(mContext, "请先选择住宅楼");
				}
				else{
					if(!selectDivisionUnitCustomDialog.isShowing()){
						selectDivisionUnitCustomDialog.show();
					}
				}
			}
		});
	...
	private void submit(){
		...
		List<ChildMeterBean> childMeterBeans = roomMeterSelectBean.getChildMeterList();
...
//					.setTitle("错误")
//					.setMessage(roomMeterSelectBean.getRoomOwnerCode() +cBean.getName()+"本期数小于上期数")
...
//				}
	...
	private void loadBuildingListHttp(){
		...
		httpFormBean.setUrl(application.getCommonHttpUrlActionManager().getOfficeMeterBuildingList());
	 ...
	 * 获取科室（单位）的表列表信息
	 */
	private void loadDivisionMeterListHttp(String houseId){
		HttpAsyncTask httpAsyncTask = new HttpAsyncTask(mContext,application.getHttpClientManager());
		HttpFormBean httpFormBean = new HttpFormBean();
		httpFormBean.setUrl(application.getCommonHttpUrlActionManager().getOfficeMeterList());
		Collection<NameValuePair> params = new ArrayList<NameValuePair>();
		NameValuePair houseIdPair = new BasicNameValuePair("houseId", houseId);
		params.add(houseIdPair);
		httpFormBean.setParams(params);
		httpAsyncTask.setOnDealtListener(new OnDealtListener() {
			...
			public void success(String resultResponse) {
				hideProgressDialog();
				ResponseBean<List<ChildMeterBean>> responseBean = application.getGson().fromJson(resultResponse, new TypeToken<ResponseBean<List<ChildMeterBean>>>(){}.getType());
				List<ChildMeterBean> robResponse =  responseBean.getData();
				if(responseBean.isSuccess() == false){
					ToastUtil.showToast(mContext, responseBean.getMessage());
					submitView.setVisibility(ViewGroup.INVISIBLE);
					return;
				...
				if(robResponse == null){
					...
					childMeterBeanList.clear();
					childMeterBeanList.addAll(robResponse);
					roomMeterSelectBean.setChildMeterList(childMeterBeanList);
					roomMeterBaseAdapter.notifyDataSetChanged();
	...
	private void loadRoomMeterListHttp(String buildingId){
		HttpAsyncTask httpAsyncTask = new HttpAsyncTask(mContext,application.getHttpClientManager());
		HttpFormBean httpFormBean = new HttpFormBean();
		httpFormBean.setUrl(application.getCommonHttpUrlActionManager().getOfficeMeterDivisionList());
		Collection<NameValuePair> params = new ArrayList<NameValuePair>();
		...
		httpAsyncTask.setOnDealtListener(new OnDealtListener() {
			...
			public void success(String resultResponse) {
				...
				if(robResponse == null){
					...
					roomMeterWheelAdapter = new RoomMeterWheelAdapter();
					selectDivisionUnitWv.setViewAdapter(roomMeterWheelAdapter);
				}
	...
	private class RoomMeterWheelAdapter implements WheelViewAdapter{
		...
		public View getItem(int index, View convertView, ViewGroup parent) {
			convertView = LayoutInflater.from(mContext).inflate(R.layout.wheel_list_item_common, null);
			TextView textView = (TextView) convertView.findViewById(R.id.wheel_item_text); 
			String name = roomMeterBeanList.get(index).getRoomOwnerName();
			textView.setText(name);
			return convertView;
		}
	...
	private class RoomMeterBaseAdapter extends BaseAdapter{
		...
		@Override
		public int getCount() {
			return roomMeterSelectBeanList.size();
		}
		...
		public View getView(int position, View convertView, ViewGroup parentView) {
			...
			RoomMeterBean roomMeterBean = roomMeterSelectBeanList.get(position);
			...
			//
			holder.roomOwnerCode.setVisibility(ViewGroup.GONE);
			holder.roomOwnerName.setVisibility(ViewGroup.GONE);
			//
	...
	private class ChildMeterBaseAdapter extends BaseAdapter{
		...
		@Override
		public int getCount() {
			Logger.i("ChildMeterBaseAdapter getCount:" +roomMeterBean.getChildMeterList().size());
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			holder.currentValueView.addTextChangedListener(new TextWatcher() {
				...
				public void afterTextChanged(Editable s) {
					...
					Logger.i("after:"+childMeterBean.getName() + childMeterBean.getDataCurrentValue());
