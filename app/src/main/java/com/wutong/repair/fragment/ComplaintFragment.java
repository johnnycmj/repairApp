...
import com.wutong.repair.activity.ComplaintSubmitActivity;
...
public class ComplaintFragment extends BaseFragment {
	...
	private String complaintTitle = "建  议";
	...
	private void loadComplaintListAsyncHttp(ComplaintPagerBean complaintPagerBean,final boolean isRefresh){
		...
		NameValuePair userIdPair = new BasicNameValuePair("userId", this.application.getLoginInfoBean().getUserId().toString());
		params.add(userIdPair);
		NameValuePair userTypePair = new BasicNameValuePair("userType", this.application.getLoginInfoBean().getUserType());
		params.add(userTypePair);
	...
	private int selectedContentView(){
		switch (application.getSkinType()) {
		case 1:
			return R.layout.spring_horse_vp_av_index_tab_complaint;

		default:
			return R.layout.vp_av_index_tab_complaint;
		}
	}
}
