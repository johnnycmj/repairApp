...
public class MoreFragment extends BaseFragment {
	...
	private void viewInit(){
		...
		for(final ModularNetworkBean modularNetworkBean : extraModularList){
			modularView = LayoutInflater.from(mContext).inflate(selectedMoreLayout(), null);
			...
			if(modularNetworkBean.getUrl().equals(getString(R.string.modular_url_person_info))){
				...
				//微分享
				textView.setText(R.string.av_index_tab_label_modular_page_more_micro_share);
			}
			else if(modularNetworkBean.getUrl().equals(getString(R.string.modular_url_room_meter))){
				//住户抄表
				textView.setText(R.string.av_index_tab_label_modular_page_more_room_meter);
			}
			else if(modularNetworkBean.getUrl().equals(getString(R.string.modular_url_office_meter))){
				//院内抄表
				textView.setText(R.string.av_index_tab_label_modular_page_more_office_meter);
			}
			
			else if(modularNetworkBean.getUrl().equals(getString(R.string.modular_url_help_found_list))){
				//我捡到了
				textView.setText(R.string.av_index_tab_label_modular_page_more_lost_or_found_list);
			}
			else if(modularNetworkBean.getUrl().equals(getString(R.string.modular_url_help_lost))){
				//寻物启事
				textView.setText(R.string.av_index_tab_label_modular_page_more_lost_list);
			}
			else if(modularNetworkBean.getUrl().equals(getString(R.string.modular_url_my_publish_lost_or_found))){
				//我发布的
				textView.setText(R.string.av_index_tab_label_modular_page_more_my_publish_lost_or_found);
			}
			else if(modularNetworkBean.getUrl().equals(getString(R.string.modular_url_repairman_like))){
				//劳动节赞
				textView.setText(R.string.av_index_tab_label_modular_page_more_repairman_like);
			}
			else if(modularNetworkBean.getUrl().equals(getString(R.string.modular_url_micro_share))){
				//微分享
				textView.setText(R.string.av_index_tab_label_modular_page_more_micro_share);
			}
			else if(modularNetworkBean.getUrl().equals(getString(R.string.modular_url_meter_change))){
				//抄表修改
				textView.setText(R.string.av_index_tab_label_modular_page_more_meter_change);
			}
			else{
	...
	private void loadCheckUpdate(){
		...
		httpAsyncTask.setOnDealtListener(new OnDealtListener() {
			...
			public void success(String resultResponse) {
				hideProgressDialog();
				if(!MoreFragment.this.isAdded() ||! MoreFragment.this.isVisible()){
					return;
				}
				...
				if(responseBean.isSuccess()){
					if(ApplicationInfoUtil.getVersionCode(mContext) < Integer.valueOf(updateBean.getVersionCode())){
						...
						//无更新
						ToastUtil.showToast(mContext, R.string.tips_for_newest_version_now);
					}
				}
				else{
	...
	private int selectedMoreLayout(){
		switch (application.getSkinType()) {
		case 1:
			return R.layout.spring_horse_index_more_extra_modular;

		default:
			return R.layout.index_more_extra_modular;
		}
	}
	...
	private int selectedContentView(){
		switch (application.getSkinType()) {
		case 1:
			return R.layout.spring_horse_vp_av_index_tab_more;

		default:
			return R.layout.vp_av_index_tab_more;
		}
	}
